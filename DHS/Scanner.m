...
-(id)initWithOptions:(NSDictionary*)options
{
    ...
    if(nil != self)
    {
        ...
        //init fps
        // ->add each to set
        for(NSUInteger i=0; i < sizeof(KNOWN_FPS)/sizeof(KNOWN_FPS[0]); i++)
        {
            //add
            [self.knownFPs addObject:KNOWN_FPS[i]];
        }

        //alloc list for all scanned binaries
...
-(BOOL)isFalsePositive:(Binary*)binary
{
    //flag
    BOOL isFalsePositive = NO;
    
    //check is just path
    if(YES == [self.knownFPs containsObject:binary.issueItem])
    {
        //set
        isFalsePositive = YES;
    }
